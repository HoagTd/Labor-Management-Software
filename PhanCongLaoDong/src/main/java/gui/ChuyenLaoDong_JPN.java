...
public class ChuyenLaoDong_JPN extends JPanel implements ActionListener, MouseListener, KeyListener {
	...
	public ChuyenLaoDong_JPN(QuanLy ql) {
		...
		modelCT = new DefaultTableModel(new String[] { "Mã công trình", "", "Ä�á»‹a Ä‘iá»ƒm" }, 0) {
			/**
			 * 
			 */
			private static final long serialVersionUID = 1L;
		...
		JLabel lblTnCngTrnh = new JLabel("");
		...
		tbLDNhan.getTableHeader().setBackground(new Color(146, 200, 240));
		tbLDNhan.getTableHeader().setForeground(new Color(255, 255, 255));
		...
		tbLDChuyen.getTableHeader().setBackground(new Color(146, 200, 240));
		tbLDChuyen.getTableHeader().setForeground(new Color(255, 255, 255));
		...
		JLabel lblTng = new JLabel("Tá»•ng:");
		...
		if (o.equals(tbCongTrinh)) {
			...
			if (validata()) {
				if (soLuongThem > 0) {
					if (soLuongThem < modelLDChuyen.getRowCount() + 1) {
						...
						JOptionPane.showMessageDialog(null, "Không đổi lao động");
					}
				} else {
					int row = tbLDChuyen.getSelectedRow();
					if (row == -1) {
			...
			if (soLuongXoa > 0) {
				if (soLuongXoa < modelLDNhan.getRowCount() + 1) {
					...
					JOptionPane.showMessageDialog(null, "QuÃ¡ sá»‘ lao Ä‘á»™ng hiá»‡n cÃ³!");
				}
			} else {
				...
				if (row >= 0) {
					...
					JOptionPane.showMessageDialog(null, "Chuyển chọn lao động");
				}
			}
