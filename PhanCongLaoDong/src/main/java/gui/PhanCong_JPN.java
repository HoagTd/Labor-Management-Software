...
public class PhanCong_JPN extends JPanel implements ActionListener, ItemListener, MouseListener, KeyListener {
	...
	public PhanCong_JPN(QuanLy ql) {
		...
		panel_3.setBorder(new TitledBorder(null, "Cong trình", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		modelCT = new DefaultTableModel(new String[] { "Tên Cong trình", "Ngày KC" }, 0) {
		...
		JLabel lblTnCngTrnh = new JLabel("Tên Cong trình");
		...
		JLabel lblTmKim = new JLabel("TÌm hiếm:");
		...
		JLabel lblSLngLao = new JLabel("Số lượng lao động đánh để phân công:");
		...
		pnLaoDong.setBackground(new Color(250, 250, 250));
		...
		modelLD = new DefaultTableModel(new String[] { "Mã lao động", "Tên lao động", "Công trình" }, 0) {
			/**
			 * 
			 */
			private static final long serialVersionUID = 1L;
		...
		tbCongTrinh.getTableHeader().setForeground(new Color(255, 255, 255));
		tbCongTrinh.getTableHeader().setBackground(new Color(146, 200, 240));
		...
		lblChiTietCT.setToolTipText("Xem chi tiáº¿t");
		...
		tbLaoDong.getTableHeader().setForeground(new Color(255, 255, 255));
		tbLaoDong.getTableHeader().setBackground(new Color(146, 200, 240));
		...
		JLabel lblTng_1 = new JLabel("Trang:");
		...
		tbPhanCong.getTableHeader().setForeground(new Color(255, 255, 255));
		tbPhanCong.getTableHeader().setBackground(new Color(146, 200, 240));
		...
		JLabel lblTng = new JLabel("Trang:");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnPhanCong)) {
			...
			if (row == -1) {
				JOptionPane.showMessageDialog(null, "Vui lòng chọn Cong trình");
				return;
			}
			...
			if (kt == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(null, "Ä�Ã£ phÃ¢n cÃ´ng");
			}
		...
		if (o.equals(tbCongTrinh)) {
			...
			if (kiemTra()) {
				if (sLThem > 0) {
					if (sLThem < modelLD.getRowCount() + 1) {
						...
						JOptionPane.showMessageDialog(null, "KhÃ´ng Ä‘á»§ lao Ä‘á»™ng!");
						return;
					}
			...
			if (sLXoa > 0) {
				if (sLXoa < modelPC.getRowCount() + 1) {
					...
					JOptionPane.showMessageDialog(null, "KhÃ´ng Ä‘á»§ sá»‘ lÆ°á»£ng lao Ä‘á»™ng Ä‘á»ƒ xÃ³a!");
					return;
				}
			} else {
	...
	public void xoaLaoDongTrung(Date ngayTH, Date ngayHT) {
		...
		new Thread(() -> {
			...
			for (int i = listLD.size()-1; i>=0;i--) {
				...
				boolean check = true;// true lÃ  xÃ³a
				dem++;
...
//							náº¿u nÃ³ náº±m giá»¯a
								check = false;
								break;
							}
						}
//						náº¿u khÃ´ng náº±m giá»¯a -> kiá»ƒm tra 2 biÃªn
						if (check) {
//						náº¿u thá»�a biÃªn bÃªn trÃ¡i
							if (ht.before(first.getNgayThucHien()) && ht.before(first.getNgayHoanThanh())) {
								check = false;
							}
//						náº¿u thá»�a biÃªn bÃªn pháº£i
